...
class CheckerUtils {
    companion object {
        private var mapper: ObjectMapper = ObjectMapper().registerModule(JavaTimeModule())
        ...
        fun checkBuilder(type: Class<*>, maxScore: Float, builtType: Class<*>? = null): Float {
            ...
            // field setters
            // Inside the loop where you check for setter methods
            for (method in methods) {
                for (field in fields) {
                    val setterMethodsForField = methods.filter { it.name.startsWith("set") && it.parameterCount == 1 && isParameterTypeMatch(it, field.type) }

                    if (setterMethodsForField.isNotEmpty()) {
                        totalFields++
                        // Choose the first matching setter method for simplicity
                        val setterMethod = setterMethodsForField[0]
            ...
            if (builtType != null) {
                ...
                if (build != null) {
                    for (method in setterMethods) {
                        val randomValue = getRandomValue(method.parameterTypes[0])
                    ...
                    for (builtField in builtFields) {
                        ...
                        val builtValue = builtField.get(built)
                        val builderValue = builderField.get(instance)

                        if (Objects.equals(builtValue, builderValue)) {
                            correctFields++
                        }
                    }
                }
        ...
        fun isReturnTypeMatch(method: Method, fieldType: Class<*>): Boolean {
            if (method.name.startsWith("set") && method.returnType == Void.TYPE) {
                // For setters, check if the parameter type matches the field type
                return method.parameterTypes.isNotEmpty() && method.parameterTypes[0].isAssignableFrom(fieldType)
            } else {
                // For getters, check if the return type matches the field type
                return method.returnType.isAssignableFrom(fieldType)
            }
        }

        private fun isParameterTypeMatch(method: Method, fieldType: Class<*>): Boolean {
            return method.parameterTypes[0] == fieldType
        }
//ORIGINAL VERS
//        fun checkBuilder(type: Class<*>, maxScore: Float, builtType: Class<*>? = null): Float {
//            var result = 0.0f
...
//            }
//
//        result += correctFields.toFloat() / totalFields.toFloat() * 0.5f
//            val constructor = type.declaredConstructors[0]
//            val cut:List<Any?> = constructor.parameters.map{ getRandomValue(it.type) }
//            val par = Array(cut.size){
//                cut[it]
...
//            println("Checking equality for class: ${classType.simpleName}")
//
//            if (first == null && second == null) {
//                println("Both objects are null. They are equal.")
//                return true
//            }
//
//            if (first == null || second == null) {
//                println("One of the objects is null. They are not equal.")
//                return false
//            }
//
//            if (classType.isPrimitive || classType.isEnum || types.contains(classType)) {
//                val result = first == second
//                println("Primitive or Enum comparison result: $result")
//                return result
//            }
//
//            val fields = getAllFields(classType)
//            val result = fields.filter { field ->
//                !Modifier.isStatic(field.modifiers)
//            }.all { field ->
//                field.isAccessible = true
//
//                // Use a workaround for java.time fields
//                if (field.type.name.startsWith("java.time")) {
//                    val accessor = FieldAccessor(field)
//                    val fieldResult = accessor.get(first) == accessor.get(second)
//                    println("Field ${field.name} comparison result: $fieldResult")
//                    fieldResult
...
//                        println("Both collections are null. They are equal.")
//                        true
//                    } else if (firstCollection == null || secondCollection == null) {
//                        println("One of the collections is null. They are not equal.")
//                        false
//                    } else {
//                        val genericType = (field.genericType as ParameterizedType).actualTypeArguments[0]
//                        val collectionResult = if (genericType is Class<*>) {
//                            containsAll(genericType, firstCollection, secondCollection)
//                        } else {
//                            firstCollection == secondCollection
//                        }
//                        println("Collection ${field.name} comparison result: $collectionResult")
//                        collectionResult
//                    }
//                } else {
//                    val fieldResult = checkEquals(field.type, field.get(first), field.get(second))
//                    println("Field ${field.name} comparison result: $fieldResult")
//                    fieldResult
//                }
//            }
//
//            println("Overall comparison result for class ${classType.simpleName}: $result")
//            return result
//        }
        ...
        fun checkEquals(classType: Class<*>, first: Any?, second: Any?): Boolean {
            ...
            return fields.filter { field -> !Modifier.isStatic(field.modifiers) }
                .all { field ->
                    ...
                    if (field.type.name.startsWith("java.time")) {
                        val accessor = FieldAccessor(field)
                        return@all accessor.get(first) == accessor.get(second)
                    }
        ...
        class FieldAccessor(private val field: Field) {
            init {
                field.isAccessible = true
            }
